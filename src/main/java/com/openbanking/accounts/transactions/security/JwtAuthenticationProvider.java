...
import com.openbanking.accounts.transactions.domain.JwtUserDetails;
...
public class JwtAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    @Override
    protected UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken) throws AuthenticationException,JwtMissingException {

        JwtAuthenticationToken jwtAuthenticationToken = (JwtAuthenticationToken) usernamePasswordAuthenticationToken;
        String token = jwtAuthenticationToken.getToken();
        log.debug("Validating jwtToken - {}", token);
        JwtUser jwtUser = validator.validate(token);

        if (jwtUser == null) {
            log.info("JWT Token is incorrect");
            throw new JwtMissingException("JWT Token is incorrect");
        }
